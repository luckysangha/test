...
(
  ...
  max_iterations = 10  -- Aggressively reduce iterations for faster training
)
AS
...
WITH features AS (
  ...
  WHERE
    totals.newVisits = 1
    AND date BETWEEN '20160801' AND '20170430'  -- Training period
),

return_visitors AS (
  SELECT
    fullVisitorId,
    COUNTIF(totals.transactions > 0 AND totals.newVisits IS NULL) > 0 AS will_buy_on_return_visit
  FROM
    `data-to-insights.ecommerce.web_analytics`
  WHERE
...
SELECT
  f.bounces,
  f.time_on_site,
  IF(rv.will_buy_on_return_visit, 1, 0) AS will_buy_on_return_visit
FROM
  features f
LEFT JOIN
  return_visitors rv
